...
const updateReservation = async (req) => {
  const { _id, email, name, date } = req.body;
  return await req.db
    .collection("calendar")
    .updateOne({ _id: new mongodb.ObjectID(_id) }, { $set: { date, email, name } })
    .then((response) => {
      return { status: "ok" };
    })
    .catch((error) => {
...
handler.put((req, res) => {
  ...
  return getByID(req).then((data) => {
    if (data && data.email === email) {
      ...
      return checkReservationExists(req, date, email)
        .then((count) => {
          if (count > 0) {
